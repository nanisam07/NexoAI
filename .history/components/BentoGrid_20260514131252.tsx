      ...
      <div className="grid md:grid-cols-3 gap-6">
        {cards.map((card, index) => (
          <motion.div
            ...
            whileHover={{ y: -8 }}
            className={`relative group overflow-hidden glass-island rounded-[2.5rem] p-8 border border-white/5 hover:border-white/20 transition-all ${card.colSpan} ${card.height}`}
          >
            {/* Unique Ambient Glow for each card */}
            <div className={`absolute -right-10 -bottom-10 w-40 h-40 ${card.bg} blur-[60px] rounded-full group-hover:scale-150 transition-transform duration-700`} />
